...
- ✅ M2: Исправлена пагинация блога — ссылки переведены на `/blog?page=N`, добавлены правила `_redirects`, проверки и ручной просмотр подтвердили корректность переходов (`logs/check_links-20251011T191038Z.json`, `logs/check_utf8-20251011T191042Z.json`).

**Следующий шаг:** Определить следующую задачу майлстоуна M (M3) и подготовить требования.
...
| M2 | Починить пагинацию блога: заменить ошибочные ссылки `blog&page=N` на корректные `/blog?page=N` во всех страницах списка. | Сначала подтвердить текущее здоровье страниц блога (`python tools/check_links.py --scope blog.html --scope 'blog&page=*.html'`, `python tools/check_utf8.py --scope blog.html --scope 'blog&page=*.html'`, ручной просмотр переходов), затем `rg "blog%26page" blog*.html`, пакетное исправление ссылок и повторные проверки/ручной просмотр 1–3 страниц, чтобы подтвердить сохранность рабочего функционала. | ✅ Выполнено — ссылки заменены на формат `/blog?page=N`, добавлен `_redirects` для раздачи статических файлов `blog&page=N.html`, обновлён `cloudflare_request_plan.jsonl`. |
...
- 2025-10-11T19:10:52Z — обновлены блоки пагинации в `blog.html`, `blog&page=2.html`…`blog&page=7.html`, а также дубликатах `index0e16.html`, `print0e16.html`; ссылки переведены на `/blog?page=N` с подсветкой активной страницы.
- 2025-10-11T19:10:56Z — добавлен `_redirects` с правилами `/blog?page=N` → `blog&page=N.html`, обновлён план HTTP-запросов `cloudflare_request_plan.jsonl` на новые URL.
- 2025-10-11T19:10:58Z — `python tools/check_links.py --scope blog.html --scope 'blog&page=*.html'` → `logs/check_links-20251011T191038Z.json`, `python tools/check_utf8.py --scope blog.html --scope 'blog&page=*.html'` → `logs/check_utf8-20251011T191042Z.json`; локально проверены страницы через `python -m http.server` + `curl -I` (`blog.html`, `blog&page=2.html`, `blog&page=3.html`).

**Сделанные микро-шаги (3/3):** обновлён список ссылок во всех версиях страниц, настроены правила `_redirects`, повторно пройдены проверки и ручной просмотр.

**Оставшиеся микро-шаги:** нет — шаг M2 закрыт, требуется сформировать следующую задачу майлстоуна M.
