...
- ▶️ D2: серии `index0*`/`print0*`, `index1*`/`print1*`, `index2*`/`print2*`, `index3*`/`print3*`, `index4*`/`print4*`, `index5*`/`print5*`, `index6*`/`print6*`, `index7*`/`print7*`, `index8*`/`print8*`, `index9*`/`print9*`, `indexa*`/`printa*` и `indexb*`/`printb*` перекодированы, впереди `indexc*`/`printc*`.

**Следующий шаг:** D2 — продолжить перекодировку серий `indexc*`/`printc*`.
