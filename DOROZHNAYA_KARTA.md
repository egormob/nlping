...
**Следующий шаг:** E2 — в среде с доступом к Cloudflare Pages выполнить `python tools/check_links.py --manifest tools/url_manifest.txt --base https://<project>.pages.dev --request-plan cloudflare_request_plan.jsonl --request-plan-format jsonl` и `python tools/check_utf8.py --manifest tools/url_manifest.txt --base https://<project>.pages.dev`, приложив отчёты к дорожной карте.
