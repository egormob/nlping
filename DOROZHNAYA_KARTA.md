...
| E2 | Деплой на Cloudflare Pages, `Purge Everything`, прогон тестов по доменам Pages и `nlping.ru`. | Проверка: `curl -I` → `charset=utf-8`, отчёты скриптов. | 🚧 В работе — локальные ассеты восстановлены, ждём проверок на Pages. |

> 🔁 **Правило удалённых проверок.** Если в рабочем контейнере нет доступа к Cloudflare Pages, ассистент обязан подготовить единый промпт для внешнего агента. В промпте должна быть ссылка на активный деплой (`https://248c2942.nlping.pages.dev`), последовательность команд `python tools/check_links.py --manifest tools/url_manifest.txt --base … --request-plan cloudflare_request_plan.jsonl --request-plan-format jsonl --output logs/check_links-remote-<timestamp>.json`, `python tools/check_utf8.py --manifest tools/url_manifest.txt --base … --output logs/check_utf8-remote-<timestamp>.json` и запуск `python scripts/qa_site_check.py > reports/<timestamp>_pages_remote_check.md`. Результат внешний агент публикует в этих файлах и завершает `cat reports/<timestamp>_pages_remote_check.md` для быстрого просмотра. Промпт передаётся оператору одним блоком без дополнительных инструкций.
...
| 2025-10-11T10:22:22Z | E2 — подготовлен регламент взаимодействия с внешним агентом | Зафиксировано правило «удалённых проверок» (см. выше) и сформирован единый промпт с командами для агента, включающий деплой `https://248c2942.nlping.pages.dev` и выходные файлы `logs/check_links-remote-<timestamp>.json`, `logs/check_utf8-remote-<timestamp>.json`, `reports/<timestamp>_pages_remote_check.md` | 🚧 Ожидаем выполнение промпта во внешней среде и загрузку логов в указанные файлы. |
