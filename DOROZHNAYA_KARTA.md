...
**Следующий шаг:** M0 — удаление блока «Бесплатные вебинары» и глушилки виджета с проверками сохранности остального функционала секции.
...
| Код | Шаг | Проверки и артефакты | Статус |
| --- | --- | --- | --- |
| M0 | Удалить блок «Бесплатные вебинары» с неактивным виджетом из голубой секции на всех страницах. | До правок снять базовые проверки (`python tools/check_links.py --scope index.html --scope blog.html --scope trainers.html` и другие страницы с секцией, `python tools/check_utf8.py` по тем же файлам, ручной просмотр страницы с виджетом), зафиксировать текущее состояние через скрин или описание. После удаления заголовка и контейнера повторить `check_links`/`check_utf8`, дополнительно пройтись `rg 'Бесплатные вебинары' -g'*.html'` и ручным просмотром нескольких страниц, чтобы подтвердить, что другие элементы секции/страницы работают как прежде. | ⬜ Не начат. |
