...
- 2025-09-30T15:48:07Z — перекодировал серию `index7*`/`print7*` (`python tools/reencode.py --paths index7*.html print7*.html --limit 200` → `logs/reencode-20250930T154807Z.json`), `rg -n "windows-1251" index7*.html print7*.html` → пусто.
- 2025-09-30T15:48:32Z — `python tools/check_utf8.py --no-manifest$(printf ' --scope %s' index7*.html print7*.html)` → `logs/check_utf8-20250930T154832Z.json`, подозрительных последовательностей и замен символов не обнаружено.
- 2025-09-30T15:48:49Z — `python -m http.server 8000` + `curl -I` по `index70d1.html`, `index77ee.html`, `print77ee.html` подтвердили корректную отдачу серии `7*` в UTF-8.

**Сделанные микро-шаги (21/?)**: перекодировка и обновление мета-тегов `index0*`/`print0*`, проверка `check_utf8` для серии `0*`, перекодировка серии `index1*`/`print1*`, проверки `check_utf8` и ручной просмотр для серии `1*`, перекодировка `index2*`/`print2*`, автоматическая и ручная проверка серии `2*`, перекодировка `index3*`/`print3*`, автоматическая и ручная проверка серии `3*`, перекодировка `index4*`/`print4*`, автоматическая и ручная проверка серии `4*`, перекодировка серии `index5*`/`print5*`, автоматическая и ручная проверка серии `5*`, перекодировка серии `index6*`/`print6*`, автоматическая и ручная проверка серии `6*`, перекодировка серии `index7*`/`print7*`, автоматическая и ручная проверка серии `7*`.

**Оставшиеся микро-шаги:** следующие серии `index8*`/`print8*` и далее по нумерации.
...
- ▶️ D2: серии `index0*`/`print0*`, `index1*`/`print1*`, `index2*`/`print2*`, `index3*`/`print3*`, `index4*`/`print4*`, `index5*`/`print5*`, `index6*`/`print6*` и `index7*`/`print7*` перекодированы, впереди `index8*`/`print8*`.

**Следующий шаг:** D2 — продолжить перекодировку серий `index8*`/`print8*`.
