...
- ▶️ D2: серии `index0*`/`print0*`, `index1*`/`print1*`, `index2*`/`print2*`, `index3*`/`print3*`, `index4*`/`print4*`, `index5*`/`print5*`, `index6*`/`print6*`, `index7*`/`print7*`, `index8*`/`print8*`, `index9*`/`print9*`, `indexa*`/`printa*`, `indexb*`/`printb*`, `indexc*`/`printc*`, `indexd*`/`printd*`, `indexe*`/`printe*`, `indexf*`/`printf*`, все кластеры GUID `0*`–`FCFECF83` перекодированы; впереди тематические лендинги и спецстраницы с устаревшей декларацией `charset=windows-1251` (`alphabet.html`, `mind-layers.html`, `superpower.html`, `tropa_sily.html`, `video.html`, `trainers.html` и т.д.).

**Следующий шаг:** D2 — собрать и перекодировать следующую серию страниц из блоков блога и тематических подборок (`blog&page=*.html`, `tricks_of_language*.html`, `coaching.html`, `erickson_hypnosis*.html`, `trainings_photo_collection.html`, `tropa_movie.html`, `reality_maker_barselona.html`, `nlp-master.html`, `subscribe_*.html`, `4Qart.html`, `pages_edit133b.html`), обновить мета-теги и пройти автоматические/ручные проверки.
