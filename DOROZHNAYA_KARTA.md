...
- ⬜ M2: Исправить пагинацию блога на формат `/blog?page=N` и подтвердить переходы между страницами (`check_links`, `check_utf8`, ручной просмотр).

**Следующий шаг:** M2 — исправление пагинации блога на формат `/blog?page=N`.
...
1. Снять и зафиксировать текущее состояние пагинации: `python tools/check_links.py --scope blog.html --scope 'blog&page=*.html'` и `python tools/check_utf8.py --scope blog.html --scope 'blog&page=*.html'`, ручной просмотр минимум трёх страниц, чтобы подтвердить, что кроме известной проблемы всё работает.
2. Сформировать список файлов пагинации (`blog.html`, `blog&page=2.html` …) командой `ls blog*` и `rg "blog%26page"`.
3. Исправить ссылки пагинации на формат `/blog?page=N`, сохранив выделение текущей страницы и относительные пути.
4. Повторно прогнать `python tools/check_links.py` и `python tools/check_utf8.py` по страницам блога, сравнить с базой и убедиться в отсутствии новых ошибок.
5. Ручным просмотром проверить минимум три страницы блога, переходы между ними и корректность подсветки активной страницы, убедившись, что остальные элементы страницы (футер, шапка, формы) работают как прежде.
6. Обновить статус шага и журнал после подтверждения работоспособности пагинации и сохранности остальных функций.
