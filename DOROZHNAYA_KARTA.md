...
### Майлстоун «Переезд на Cloudflare» (закрыт)

> Шаги A–E привели зеркало к корректной структуре, перекодировали контент в UTF-8 и настроили проверки. Подробный журнал остаётся ниже как архив выполненного майлстоуна.
...
| Код | Шаг | Проверки и артефакты | Статус |
| --- | --- | --- | --- |
| E1 | Прогон локальных smoke-тестов (HTTP-сервер + `tools/check_links.py`, `tools/check_utf8.py`). | Проверка: отчёты без ошибок. | ✅ Выполнено — `tools/check_links.py --manifest tools/url_manifest.txt` → `logs/check_links-20251008T190222Z.json`, `tools/check_utf8.py --manifest tools/url_manifest.txt` → `logs/check_utf8-20251008T190357Z.json`, ручные `curl -I` по `index.html`, `blog.html`, `training10k.html`. |
| E2 | Деплой на Cloudflare Pages, `Purge Everything`, прогон тестов по доменам Pages и `nlping.ru`. | Проверка: `curl -I` → `charset=utf-8`, отчёты скриптов. | 🚧 В работе — локальные ассеты восстановлены, ждём проверок на Pages от внешнего наблюдателя. |

> 🔁 **Правило удалённых проверок.** Если в рабочем контейнере нет доступа к Cloudflare Pages, ассистент готовит бриф для внешнего наблюдателя. Наблюдатель работает только из браузера, не использует пароли и не может менять репозиторий, поэтому бриф включает:
> - ссылку на активный деплой (`https://248c2942.nlping.pages.dev`) и список ключевых страниц (`/`, `/blog.html`, `/training10k.html`, `/webinar.nlping.ru/index.html`, `/w.nlping.ru/index.html`) для проверки читаемости текста и корректной отдачи изображений;
> - инструкции воспользоваться публичным HTTP-инспектором без авторизации (например, `https://httpstatus.io/` или `https://www.webconfs.com/http-header-check.php`) для фиксирования заголовков `Content-Type`/`charset` по доменам Pages и `nlping.ru` после деплоя;
> - просьбу открыть исходный код (`View Source`) каждой страницы и убедиться, что в `<meta charset>` указан `utf-8`;
> - просьбу зафиксировать результаты в ответном сообщении (URL, время проверки, заголовки, заметки по отображению, при возможности — публичные скриншоты/ссылки). Получив отчёт, мы переписываем выводы в `logs/` и `reports/` самостоятельно.
...
| 2025-10-08T19:02:22Z | E1 — локальные smoke-тесты и корректировка `AssetCollector` | `python tools/check_links.py --manifest tools/url_manifest.txt` → `logs/check_links-20251008T190222Z.json` (обновлён разбор `../`-ссылок в `tools/list_assets.py`); `python tools/check_utf8.py --manifest tools/url_manifest.txt` → `logs/check_utf8-20251008T190357Z.json`; `curl -I http://localhost:8000/{index.html,blog.html,training10k.html}` | ✅ Проверки зафиксированы; `check_links` теперь корректно обрабатывает переходы к корню, но выявил 79 отсутствующих ассетов (в основном `s.nlping.ru/jsapi/click.js`, favicon и статические файлы `ark.ru`) — требуется разбор на этапе E2. |
| 2025-10-08T19:50:37Z | E2 — восстановил отсутствующие ассеты перед деплоем | `python tools/check_links.py --manifest tools/url_manifest.txt` → `logs/check_links-20251008T195037Z.json`; добавлены заглушки `s.nlping.ru/jsapi/click.js`, директория `/favicon`, восстановлены Drupal- и WordPress-ресурсы из `webinar.nlping.ru/` | 🚧 Локальный прогон без 404, следующий шаг — повторить проверки на Cloudflare Pages. |
| 2025-10-08T20:22:37Z | E2 — подтвердил отсутствие пропавших ассетов после восстановления | `python tools/check_links.py --manifest tools/url_manifest.txt` → `logs/check_links-20251008T202237Z.json`; `summary.assets_missing=0`, `summary.documents_missing=0`; подготовлена выборка для удалённого прогона через `--base https://<project>.pages.dev` | 🚧 Готово к запуску HTTP-проверок на Pages, требуется доступ к облачной среде. |
| 2025-10-09T07:52:44Z | E2 — подготовил оффлайн-план HTTP-запросов и зафиксировал сетевое ограничение | `python tools/check_links.py --manifest tools/url_manifest.txt --request-plan cloudflare_request_plan.jsonl --request-plan-format jsonl --output logs/check_links-local.json`; попытка `curl -I https://nlping.pages.dev` возвращает `HTTP/1.1 403 Forbidden` → фиксируем, что прямой выход в интернет заблокирован | 🚧 План HTTP-запросов лежит в `cloudflare_request_plan.jsonl`, для запуска на Pages нужно повторить команды с реальным `--base` в среде с доступом. |
| 2025-10-09T12:27:52Z | E2 — подтвердил ограничение на исходящие HTTP-запросы при попытке проверки Pages | `python tools/check_links.py --scope index.html --base https://nlping.pages.dev --timeout 5 --output logs/check_links-remote-test.json`; `python tools/check_utf8.py --scope index.html --base https://nlping.pages.dev --no-manifest --output logs/check_utf8-remote-test.json` → обе команды завершаются ошибками `Tunnel connection failed: 403 Forbidden` в HTTP-блоках логов | ⚠️ HTTP-тесты в текущей среде невозможны, требуется повторить команды в окружении с доступом к Cloudflare Pages. |
| 2025-10-09T13:18:18Z | E2 — обновил план HTTP-запросов перед удалённым прогоном | `python tools/check_links.py --manifest tools/url_manifest.txt --request-plan cloudflare_request_plan.jsonl --request-plan-format jsonl --output logs/check_links-plan-refresh.json` → `summary.documents_missing=0`, `summary.assets_missing=0`; подтверждён объём 2889 HTTP-запросов для запуска на Pages | 🚧 План актуализирован, остаётся повторить проверки в среде с сетевым доступом. |
| 2025-10-10T13:44:15Z | E2 — повторил HTTP-прогоны для `index.html` и зафиксировал блокировку туннеля | `python tools/check_links.py --scope index.html --base https://nlping.pages.dev --timeout 5 --output logs/check_links-remote-index-20251010T134352Z.json`; `python tools/check_utf8.py --scope index.html --no-manifest --base https://nlping.pages.dev --output logs/check_utf8-remote-index-20251010T134401Z.json` → оба отчёта содержат `error: "Tunnel connection failed: 403 Forbidden"` | ⚠️ Требуется запустить полные проверки в окружении с прямым доступом к Cloudflare Pages. |
| 2025-10-10T14:40:31Z | E2 — повторная попытка HTTP-прогона `check_links` из офлайновой среды | `python tools/check_links.py --scope index.html --base https://nlping.pages.dev --timeout 5 --output logs/check_links-remote-test-1760107211.json` → для 37 ассетов зафиксирован `error: "Tunnel connection failed: 403 Forbidden"`, доступ к Cloudflare Pages по-прежнему закрыт | ⚠️ Полный прогон по манифесту невозможен без сетевого доступа; требуется запуск в среде с разрешёнными HTTP-запросами. |
| 2025-10-11T10:22:22Z | E2 — подготовлен регламент взаимодействия с внешним наблюдателем | Зафиксировано обновлённое правило «удалённых проверок» (см. выше): наблюдатель использует только браузер без паролей и не вносит изменения в репозиторий; бриф включает перечень страниц, публичные HTTP-инспекторы и формат текстового отчёта | 🚧 Ждём отчёт наблюдателя, после получения перенесём выводы в `logs/` и `reports/`. |
| 2025-10-11T10:35:13Z | E2 — оформлен бриф для внешнего наблюдателя | Создан файл `reports/2025-10-11_pages_observer_brief.md` с перечнем URL, инструкциями по фиксации заголовков `Content-Type`, проверке `<meta charset="utf-8">`, шаблоном ответа и описанием дальнейших шагов после получения отчёта | 🚧 Ожидаем обратную связь наблюдателя и готовим площадку для переноса результатов в `logs/` и `reports/`. |
| 2025-10-11T10:50:00Z | E2 — получен частичный отчёт наблюдателя по домену `pages.dev` | Отчёт `reports/2025-10-11_pages_dev_manual_check.md` фиксирует ручной просмотр 5 страниц `https://248c2942.nlping.pages.dev`, подтверждён UTF-8 в рендере и `<meta charset="utf-8">`; HTTP-заголовки не сняты из-за недоступности внешних чекеров, зеркальная проверка `https://nlping.ru` отсутствует | 🚧 Требуется дополнить отчёт заголовками `Content-Type` и проверить домен `https://nlping.ru`. |
...
| 2025-10-11T17:43:22Z | M0 — удалён блок «Бесплатные вебинары» на 620 страницах | До/после: `python tools/check_links.py --scope index.html --scope blog.html --scope trainers.html` → логи `logs/check_links-20251011T174231Z.json`, `logs/check_links-20251011T174318Z.json`; `python tools/check_utf8.py --scope index.html --scope blog.html --scope trainers.html` → логи `logs/check_utf8-20251011T174233Z.json`, `logs/check_utf8-20251011T174322Z.json`; `rg "Бесплатные вебинары" -g'*.html'` → пусто | ✅ Секция очищена, остальные элементы страниц сохранены. |
...
- ✅ E1: локальные smoke-тесты выполнены (`logs/check_links-20251008T190222Z.json`, `logs/check_utf8-20251008T190357Z.json`, ручные `curl -I` по ключевым страницам). `check_links` фиксирует 79 отсутствующих ассетов (скрипт `s.nlping.ru/jsapi/click.js`, фавиконки `favicon/`, статические ресурсы `ark.ru` и др.) — план разбора включён в шаг E2.

#### Майлстоун «Исправление улучшение сайта» (активен)

- ✅ M0: Удалить блок «Бесплатные вебинары» с неработающим виджетом и убедиться, что остальные элементы секции работают как прежде (`rg`/ручной просмотр перед правками, `check_links`/`check_utf8`/ручной просмотр до и после).
- ✅ M1: Удалены блоки Perfect Audience и AdRoll, футеры очищены; подтверждено `logs/check_links-20251011T183110Z.json`, `logs/check_utf8-20251011T183113Z.json`, просмотр исходников `index.html`, `blog.html`, `trainers.html`.
- 🚧 M2: Исправить визуальный дефект перекрытия контента на страницах, где блок под статьёй накладывается на основной текст и искажает кодировку.
  - Выяснить, в чём проблема: воспроизвести дефект, снять скриншоты/HTML-фрагменты, определить конфликтующие блоки и условия появления.
  - Определить, системная ли проблема: проверить другие страницы и шаблоны (блог, индекс, печатные версии), собрать список затронутых файлов.
  - Найти решение: подобрать HTML/CSS-исправления или порядок элементов, который устраняет перекрытие и сохраняет корректную кодировку (учесть print-версии).
  - Применить решение: внести правки на всех затронутых страницах, повторить автоматические проверки (`check_links`, `check_utf8`) и ручной просмотр.
- 🚧 M3: Перепроектировать пагинацию блога, чтобы избавиться от `%26page%3dN` в ссылках и при этом не ломать переходы на Cloudflare Pages; вариант с `/blog?page=N` отклонён — `_redirects` не обрабатывает query string.

**Следующий шаг:** M2 — диагностировать и устранить перекрытие контента, затем вернуться к обновлению пагинации в рамках M3.
...
| Код | Шаг | Проверки и артефакты | Статус |
| --- | --- | --- | --- |
| M0 | Удалить блок «Бесплатные вебинары» с неактивным виджетом из голубой секции на всех страницах. | До правок снять базовые проверки (`python tools/check_links.py --scope index.html --scope blog.html --scope trainers.html` и другие страницы с секцией, `python tools/check_utf8.py` по тем же файлам, ручной просмотр страницы с виджетом), зафиксировать текущее состояние через скрин или описание. После удаления заголовка и контейнера повторить `check_links`/`check_utf8`, дополнительно пройтись `rg 'Бесплатные вебинары' -g'*.html'` и ручным просмотром нескольких страниц, чтобы подтвердить, что другие элементы секции/страницы работают как прежде. | ✅ Выполнено — блок удалён на 620 страницах, подтверждено `logs/check_links-20251011T174231Z.json` → `...T174318Z.json`, `logs/check_utf8-20251011T174233Z.json` → `...T174322Z.json`, `rg "Бесплатные вебинары" -g'*.html'` → пусто. |
| M1 | Удалить текстовый вывод скриптов ретаргетинга из футера и восстановить чистый блок контактов. | Перед правками зафиксировать базовое состояние (`python tools/check_links.py --scope футерных страниц`, `python tools/check_utf8.py --scope футерных страниц`, ручной просмотр), затем поиск `rg 'perfectaudience' -g'*.html'`, обновление HTML с переносом скриптов в `<script>` без текстового вывода или удалением, повторный прогон тех же проверок и ручной просмотр 3 страниц футера, чтобы убедиться в отсутствии регрессий. | ✅ Выполнено — автоматическим скриптом удалены блоки Perfect Audience и AdRoll на 1009 страницах, проверки `logs/check_links-20251011T183110Z.json`, `logs/check_utf8-20251011T183113Z.json`, просмотр исходников `index.html`, `blog.html`, `trainers.html` подтверждает чистый футер. |
| M2 | Исправить перекрытие текста основного контента блоком рекомендаций/форм, проявившееся после последних правок. | 1. Выяснить, в чём проблема: воспроизвести дефект на примере из отчёта, собрать скриншоты, DOM и CSS. 2. Определить, системная ли проблема: обойти связанные шаблоны (`blog.html`, `blog&page=N.html`, индекс/print-версии) и составить перечень затронутых страниц. 3. Найти решение: подготовить правки структуры или стилей, проверить, что кодировка отображается корректно. 4. Применить решение: внести изменения во все затронутые файлы, прогнать `python tools/check_links.py`/`python tools/check_utf8.py` и вручную подтвердить отсутствие перекрытия. | ✅ Выполнено — устранено перекрытие на 54 страницах, подтверждено `logs/reencode-20251019T175933Z.json`, `logs/check_utf8-20251019T180016Z.json`, ручными скриншотами `index3797_after.png`, `index0a49_after.png` и регресс-тестом `pytest tools/tests/test_reencode.py`. |
| M3 | Перепроектировать пагинацию блога без зависимости от ссылок вида `blog%26page%3dN.html`, сохранив рабочие переходы между страницами. | Подтвердить текущее здоровье страниц блога (`python tools/check_links.py --scope blog.html --scope 'blog&page=*.html'`, `python tools/check_utf8.py --scope blog.html --scope 'blog&page=*.html'`, ручной просмотр переходов). Исследовать поддержку альтернативных форматов ссылок: директные `blog&page=N.html` с HTML-экранированием, псевдокаталоги (`/blog/page/N/`), `_routes.json` или Workers. Зафиксировать результаты тестов (локально и на Pages), подобрать рабочий вариант и обновить навигацию + проверки. | 🚧 В работе — первый подход с `/blog?page=N` провалился: `_redirects` Cloudflare Pages игнорирует query string, из-за чего получались 404. |

#### Шаг M0 — Удаление блока «Бесплатные вебинары»

1. Снять базовое состояние: выполнить `python tools/check_links.py --scope index.html --scope blog.html --scope trainers.html` (при необходимости расширить список страниц с голубой секцией), `python tools/check_utf8.py` по тем же страницам и вручную открыть их через `python -m http.server`, чтобы убедиться, что текущие рабочие элементы (ссылки, формы, кнопки) функционируют.
2. Найти HTML-фрагменты с заголовком «Бесплатные вебинары» и контейнером неработающего виджета (`rg "Бесплатные вебинары" -n`).
3. Удалить заголовок и содержимое блока (заглушку/iframe/скрипты) так, чтобы секция выглядела чистой; при необходимости подправить отступы или высоту секции, не затрагивая другие элементы страницы.
4. Повторить `python tools/check_links.py` и `python tools/check_utf8.py` по исходному списку страниц, дополнительно убедиться `rg "Бесплатные вебинары" -g'*.html'` → пусто.
5. Провести ручной просмотр тех же страниц, убедиться, что остальные элементы секции и страницы работают как прежде.
6. Зафиксировать результаты (логи проверок, заметки по визуальной проверке) и обновить статус шага в дорожной карте.

#### Шаг M1 — Удаление текстовых блоков скриптов из футера

1. Снять базовое состояние без изменений: прогнать `python tools/check_links.py` и `python tools/check_utf8.py` по ключевым страницам с футером (минимум главная, блог, раздел «Контакты»), зафиксировать отсутствие ошибок и сделать ручной просмотр этих страниц, чтобы подтвердить, что рабочие элементы футера сейчас функционируют.
2. Найти все вхождения скриптов Perfect Audience/AdRoll (`rg "perfectaudience" -n`).
3. Для затронутых HTML-файлов перенести код в корректные `<script>`-теги или полностью удалить, чтобы в футере не отображался текст JavaScript. Если скрипт не нужен, оставить комментарий о его отключении.
4. Повторно запустить `python tools/check_links.py` и `python tools/check_utf8.py` по тому же набору страниц, сравнить с базой и убедиться, что рабочие элементы футера остались исправны.
5. Просмотреть страницы вручную (через `python -m http.server`) и подтвердить отсутствие текста скриптов и сохранность всех функций футера (контакты, ссылки, кнопки).
6. Зафиксировать результат в журнале прогресса и обновить статус шага, приложив ссылки на логи до и после.

- 2025-10-11T18:31:15Z — удалил блоки Perfect Audience и AdRoll из 1009 HTML-файлов (скрипт Python по `tag.perfectaudience.com` и `adroll_adv_id`), повторно запустил `python tools/check_links.py --scope index.html --scope blog.html --scope trainers.html` → `logs/check_links-20251011T183110Z.json`, `python tools/check_utf8.py --scope index.html --scope blog.html --scope trainers.html` → `logs/check_utf8-20251011T183113Z.json`; просмотр исходников `index.html`, `blog.html`, `trainers.html` подтвердил отсутствие текстового вывода JS в футере.

- 2025-10-12T09:25:00Z — протестировал формат `/blog?page=N`, добавив `_redirects`; на Cloudflare Pages запросы с query string не переписываются, в результате получались 404. Правки откатил, вернул ссылки `blog%26page%3dN.html`. Следующий шаг — подобрать формат без query string, поддерживаемый статическим хостингом (например, каталоги `/blog/page/N/` или прямые ссылки `blog&amp;page=N.html`).

#### Шаг M2 — Диагностика и исправление перекрытия контента

1. **Выяснить, в чём проблема.** Воспроизвести визуальный дефект на локальном сервере (`python -m http.server`), зафиксировать скриншоты и HTML-структуру, определить конфликтующие блоки (рекомендации, формы, баннеры), оценить влияние порядка элементов и стилей.
2. **Определить, системная ли проблема.** Проверить связанные страницы и шаблоны: `blog.html`, `blog&page=N.html`, индексные и печатные версии, тематические лендинги. Составить перечень всех файлов, где проявляется перекрытие или нарушенная кодировка.
3. **Найти решение.** Подготовить правки CSS/HTML (например, `clear`, `overflow`, `z-index`, корректное закрытие тегов, перестановка блоков). Проверить, что выбранный подход устраняет дефект и не ломает верстку.
4. **Применить решение.** Внести изменения во все затронутые файлы партиями, после каждой партии запускать `python tools/check_links.py` и `python tools/check_utf8.py`, выполнять ручной просмотр (включая страницу из отчёта). Зафиксировать скриншоты «после» и обновить статус.

- 2025-10-19T17:59:33Z — перекодировал 54 HTML-файла с артефактами Windows-1251 (см. `logs/reencode-20251019T175933Z.json`), обновил `tools/reencode.py` и тест `tools/tests/test_reencode.py`, добавил `clear` в `.bottom-block` (`css/styles.css`), проверил партию `python tools/check_utf8.py --scope <список>` → `logs/check_utf8-20251019T180016Z.json`, сделал скриншоты `index3797_after.png`, `index0a49_after.png`, запустил `pytest tools/tests/test_reencode.py`.

#### Шаг M3 — Корректировка пагинации блога

1. Зафиксировать текущее рабочее состояние (ссылки вида `blog%26page%3dN.html`): `python tools/check_links.py --scope blog.html --scope 'blog&page=*.html'` и `python tools/check_utf8.py --scope blog.html --scope 'blog&page=*.html'`, ручной просмотр минимум трёх страниц.
2. Исследовать доступные варианты переписывания ссылок. Проверить, как Cloudflare Pages обрабатывает HTML-экранирование `blog&amp;page=N.html`, каталоги `/blog/page/N/`, правила `_redirects`/`_routes.json` и Workers. Задокументировать, какие подходы работают локально и что из этого применимо на Pages.
3. Подобрать целевой формат ссылок (например, человеко-понятный путь без query string) и подготовить план миграции: какие файлы править, какие дополнительные ресурсы (redirects, `_routes.json`, Worker) потребуются.
4. Внести изменения партиями, после каждой партии повторять `check_links`/`check_utf8` по затронутым страницам и ручную проверку переходов.
5. После обновления навигации зафиксировать логи проверок, добавить заметку о выбранном решении и обновить статус шага.
