...
- **Merge после ключевых проверок.** Сразу после завершения B3 (появления `tools/check_utf8.py` и свежих логов) обновлять/сливать PR `work → main`, чтобы последующие партии оставались малыми.
...
| B3 | Скрипт `tools/check_utf8.py` — валидация кодировки, поиск `�`, сравнение SEO-блоков со слепком. | Проверка: отчёт `logs/check_utf8-*.json`. | ✅ Выполнено — скрипт собирает цели из manifest/`--scope`, проверяет HTTP-заголовки и SEO-блоки, лог лежит в `logs/check_utf8-*.json`. |
...
- ✅ B3: `tools/check_utf8.py` проверяет кодировку, `logs/check_utf8-*.json` фиксируют SEO-отклонения.
- ⏳ Остальные шаги (C1–E4) не начаты.

Следующий шаг: довести до merge актуальный PR (`work → main`) и затем перейти к C1 — переносу корневых HTML в корень репозитория.

## Быстрый старт сессии

Каждую новую сессию начинай с команды **«Протокол, шаг <код этапа>»**. После этой фразы ассистент автоматически:

1. Открывает эту дорожную карту и определяет актуальный подпункт.
2. Заглядывает в `INSTRUKTSIYA_VYPOLNENIYA.md`, чтобы следовать чек-листу на текущем этапе.
3. Подтягивает связанные заметки из каталога `memory-bank/` (например, MB-02 и MB-06), чтобы помнить о проверках и ограничениях.

Если браузер завис или контекст обнулился, просто повтори тот же короткий промпт — все документы снова откроются в нужном порядке.
