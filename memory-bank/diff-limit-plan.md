...
| --- | --- | --- | --- | --- | --- |
| 2025-10-11T17:45Z / Этап M0 | Удаление HTML блока | 620 страниц (`index*.html`, GUID `*.html`, лендинги) | 620 files, 620 insertions(+), 8680 deletions(-) | ✅ | Массово удалён iframe «Бесплатные вебинары» (скрипт `re.sub` по `<td class="left-content bottom">`), проверки `check_links`/`check_utf8` до/после и `rg "Бесплатные вебинары" -g'*.html'` подтвердили чистоту. |
| 2025-10-09T08:00Z / Инцидент diff-limit | Массовая перекодировка HTML + assets | `*` (вся оставшаяся выдача) | >5000 файлов, десятки МБ | ❌ | Попытка прогнать всю оставшуюся перекодировку одной командой `git add .` → в коммите оказалось почти всё зеркало. Платформа не смогла построить diff (ошибка «generated diff exceeds size limit»). Вывод: держаться планки ≤15 HTML на партию и не смешивать с бинарными каталогами. |
| 2025-10-08T09:56Z / Этап D2 тематические лендинги | Перекодировка HTML | `4Qart.html`, `blog&page=2.html`…`blog&page=7.html`, `coaching.html`, `erickson_hypnosis*.html`, `nlp-master.html`, `pages_edit133b.html`, `reality_maker_barselona.html`, `subscribe_*.html`, `trainings_photo_collection.html`, `tricks_of_language*.html`, `tropa_movie.html` | 22 files, 1448 insertions(+), 1447 deletions(-) | ✅ | Партия из тематических страниц с mojibake прошла в лимите; логи `logs/reencode-20251008T095552Z.json`, `logs/check_utf8-20251008T095618Z.json`, charset синхронизирован. |
...
- `fail_bound_html_guid`: ≈5000 файлов (массовая партия 2025-10-09 привела к ошибке diff-limit; использовать как верхний запрет и держаться значительно ниже).

### План после инцидента от 2025-10-09

1. **Восстановить контроль.** Перед любой новой партией фиксировать `git diff --stat` после выборочного `git add -p` и убеждаться, что в staged-области ≤15 HTML-файлов и нет бинарных каталогов.
2. **Протестировать на малой партии.** Следующая рабочая задача — выбрать один подкаталог `setup40xx.html` (до 10 файлов), перекодировать и прогнать `tools/check_utf8.py --scope <файл>`.
3. **Обновить журнал.** После успешной малой партии записать фактический объём и скорректировать `safe_bound`/`fail_bound` при необходимости.
