...
3. Повторить проверки по `https://<project>.pages.dev/` и `https://nlping.ru/` (если домен подключен). Использовать `curl -I` с `Cache-Control: no-cache`.
   - Если прямой доступ к Pages недоступен, подготовь бриф для внешнего наблюдателя: он работает только из браузера, не использует пароли и не меняет репозиторий. Укажи ссылку на деплой `https://248c2942.nlping.pages.dev`, список ключевых страниц (`/`, `/blog.html`, `/training10k.html`, `/webinar.nlping.ru/index.html`, `/w.nlping.ru/index.html`), предложи воспользоваться публичными HTTP-инспекторами (например, `https://httpstatus.io/`) для фиксации заголовков и попроси проверить `<meta charset="utf-8">` через `View Source`. Результаты наблюдатель присылает текстом, после чего перенеси данные в `logs/` и `reports/` сам.
